...
with app.app_context():
    # Suppression des anciens triggers/fonctions
    db.session.execute(db.text("DROP TRIGGER IF EXISTS after_tenant_insert ON tenants"))
    db.session.execute(db.text("DROP FUNCTION IF EXISTS insert_default_categories()"))
    db.session.execute(db.text("DROP TRIGGER IF EXISTS after_category_insert ON salon_categories"))
    db.session.execute(db.text("DROP FUNCTION IF EXISTS insert_preset_services()"))
    ...
    # Fonction pour insérer les services prédéfinis
    db.session.execute(db.text("""
