...
# Plus de template_folder pour éviter toute confusion. On utilise le dossier global templates/
...
def dashboard():
    ...
    return render_template('superadmin/index.html', tenants=data)   # Chemin complet
...
def tenant_create():
    if request.method == 'POST':
        ...
        if Tenant.query.filter_by(slug=slug).first():
            ...
            return redirect(url_for('superadmin.tenant_create'))
        tenant = Tenant(slug=slug, nom=nom, phone=phone, ville=ville, adresse_text=adresse, active=True, created_at=datetime.utcnow(), last_payment_date=datetime.utcnow())
        ...
        db.session.commit()
        log = Log(tenant_slug=slug, event_type='tenant_created', message=f'Tenant {nom} créé par superadmin {current_user.email}')
        db.session.add(log)
        db.session.commit()
        flash(f'Tenant {nom} créé avec succès. Login admin : {email} / mdp : {password}', 'success')
        return redirect(url_for('superadmin.dashboard'))
    return render_template('superadmin/tenant_create.html')   # Chemin complet
...
def tenant_logs(tenant_id):
    ...
    logs_data = [{'event': log.event_type, 'message': log.message, 'date': log.created_at.strftime('%d/%m/%Y %H:%M')} for log in logs]
