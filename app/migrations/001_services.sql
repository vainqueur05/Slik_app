-- Supprime l'ancienne table services
DROP TABLE IF EXISTS services CASCADE;

-- Recrée la table services avec colonnes pour les photos
...
-- Table des catégories activées par salon
...
-- Trigger : après insertion d’un salon, ajoute les 9 catégories par défaut (ajout Coiffure Homme)
...
BEGIN
    INSERT INTO salon_categories (tenant_slug, categorie) VALUES
        ...
        (NEW.slug, 'Coiffure Mariée'),
        (NEW.slug, 'Barber'),
        (NEW.slug, 'Coiffure Homme'),   -- nouvelle catégorie
        (NEW.slug, 'Make-up'),
        (NEW.slug, 'Onglerie'),
...
-- Trigger : insère les services prédéfinis pour chaque catégorie
...
BEGIN
    ...
    -- Coiffure Mariée
    IF NEW.categorie = 'Coiffure Mariée' THEN
        INSERT INTO services (tenant_slug, categorie, nom, prix, duree, is_preset, is_vip, actif) VALUES
            (NEW.tenant_slug, 'Coiffure Mariée', '3 coiffures', '150$', '5h', TRUE, TRUE, TRUE),
            (NEW.tenant_slug, 'Coiffure Mariée', '2 coiffures', '100$', '3h', TRUE, FALSE, TRUE),
            (NEW.tenant_slug, 'Coiffure Mariée', '1 coiffure', '50$', '2h', TRUE, FALSE, TRUE),
            (NEW.tenant_slug, 'Coiffure Mariée', 'Forfait 3 coiff + 3 make-up', '180$', '6h', TRUE, TRUE, TRUE),
            (NEW.tenant_slug, 'Coiffure Mariée', 'Forfait VIP Complet', '400$', 'journée', TRUE, TRUE, TRUE);
    END IF;

    -- Barber (services de barbier)
    IF NEW.categorie = 'Barber' THEN
        INSERT INTO services (tenant_slug, categorie, nom, prix, duree, is_preset, actif) VALUES
            (NEW.tenant_slug, 'Barber', 'Coupe Classique', '5.000FC', '20min', TRUE, TRUE),
            (NEW.tenant_slug, 'Barber', 'Dégradé Américain', '7.000FC', '30min', TRUE, TRUE),
    ...
    -- Coiffure Homme (coupes spécifiques sans barbe)
    ...
    IF NEW.categorie = 'Make-up' THEN
        INSERT INTO services (tenant_slug, categorie, nom, prix, duree, is_preset, actif) VALUES
            ...
            (NEW.tenant_slug, 'Make-up', 'Soirée', '20$', '45min', TRUE, TRUE),
            (NEW.tenant_slug, 'Make-up', 'Nude', '10$', '30min', TRUE, TRUE),
            (NEW.tenant_slug, 'Make-up', 'Mariée Essai+JourJ', '80$', '2h', TRUE, TRUE);
    END IF;
    ...
    IF NEW.categorie = 'Onglerie' THEN
        INSERT INTO services (tenant_slug, categorie, nom, prix, duree, is_preset, actif) VALUES
            ...
            (NEW.tenant_slug, 'Onglerie', 'Pédicure Spa', '15$', '1h', TRUE, TRUE);
    END IF;
